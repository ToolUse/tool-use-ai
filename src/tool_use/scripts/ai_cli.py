...
import subprocess
import sys
from tool_use.utils.ai_service import AIService
...
def main(args=None):
    ...
    # First parse known args to get the service and model options
    parser.add_argument(
        "--service",
    ...
    # Join all remaining arguments as the input text
    input_text = " ".join(unknown_args) if unknown_args else ""

    env_info = get_environment_info()
    command = query_ai_service(
        input_text, known_args.service, known_args.model, env_info
    )
